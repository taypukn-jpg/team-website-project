document.addEventListener('DOMContentLoaded', () => {
    ...
    // Автоплей слайдера
    let autoId = setInterval(() => showSlide(slideIndex + 1), 5000);
    const slideContainer = document.querySelector('.slide-container');
    slideContainer.addEventListener('mouseenter', () => clearInterval(autoId));
    slideContainer.addEventListener('mouseleave', () => autoId = setInterval(() => showSlide(slideIndex + 1), 5000));
    ...
    // Клик по карточке добавляет в избранное
    ...
    const translations = {
        kk: {
            ...
            slides: slides.map(s => ({caption: s.caption}))
        },
        ru: {
    ...
    function updateLanguage(lang) {
        ...
        places.forEach(place => {
            const name = place.dataset.name;
            place.querySelector('p').textContent = data.places[name] || place.querySelector('p').textContent;
        });
