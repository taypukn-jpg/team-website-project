// assets/js/main.js
document.addEventListener('DOMContentLoaded', () => {
  // ==================== ПЕРЕМЕННЫЕ ====================
  let currentLang = 'kk';
  let slideIndex = 0;
  const langButtons = document.querySelectorAll('.lang-btn');
  const themeButtons = document.querySelectorAll('.theme-btn');
  const places = document.querySelectorAll('.place');
  const favList = document.getElementById('fav-list');
  const authorsBtn = document.getElementById('authors-btn');
  const authorsModal = document.getElementById('authors-modal');
  const closeBtn = document.querySelector('.close-btn');
  const galleryTitleEl = document.getElementById('gallery-title');

  // ========== ТЕКСТЫ И ДАННЫЕ ==========
  const galleryText = {
    kk: {
      "Алтай таулары": "Алтай таулары – керемет табиғат көрінісі.",
      "Бурабай": "Бурабай – Қазақстанның «Кіші Швейцариясы».",
      "Шарын шатқалы": "Шарын шатқалы – тарихи және табиғи ескерткіш.",
      "Көлсай көлдері": "Көлсай көлдері – таулы көлдер тізбегі.",
      "Алматы": "Алматы – мәдени және туристік орталық.",
      "Көлтаз": "Көлтаз – тыныш табиғат аймағы.",
      "Астана": "Астана – заманауи сәулет және саябақтар.",
      "Қаратау": "Қаратау – Қазақстанның әсем таулы аймағы.",
      "Байқоңыр": "Байқоңыр – ғарыш айлағы мен тарих."
    },
    ru: {
      "Алтай таулары": "Алтайские горы — великолепные природные пейзажи.",
      "Бурабай": "Бурабай — «Маленькая Швейцария» Казахстана.",
      "Шарын шатқалы": "Шарынский каньон — природный и исторический памятник.",
      "Көлсай көлдері": "Озёра Кольсай — горная цепь озёр.",
      "Алматы": "Алматы — культурный и туристический центр.",
      "Көлтаз": "Көлтаз — спокойная природная зона.",
      "Астана": "Астана — современная архитектура и парки.",
      "Қаратау": "Каратау — красивый горный район Казахстана.",
      "Байқоңыр": "Байконур — космодром и исторический объект."
    },
    en: {
      "Алтай таулары": "Altai Mountains — a stunning natural landscape.",
      "Бурабай": "Burabay — Kazakhstan’s ‘Little Switzerland’.",
      "Шарын шатқалы": "Charyn Canyon — a natural and historical monument.",
      "Көлсай көлдері": "Kolsai Lakes — a chain of mountain lakes.",
      "Алматы": "Almaty — a cultural and tourist center.",
      "Көлтаз": "Koltaz — a peaceful natural area.",
      "Астана": "Astana — modern architecture and parks.",
      "Қаратау": "Karatau — a beautiful mountain region of Kazakhstan.",
      "Байқоңыр": "Baikonur — spaceport and history."
    }
  };

  const sliderText = {
    kk: [
      "Алтай таулары – керемет табиғат көрінісі.",
      "Бурабай – Қазақстанның «Кіші Швейцариясы».",
      "Шарын шатқалы – тарихи және табиғи ескерткіш."
    ],
    ru: [
      "Алтайские горы — великолепные природные пейзажи.",
      "Бурабай — «Маленькая Швейцария» Казахстана.",
      "Шарынский каньон — природный и исторический памятник."
    ],
    en: [
      "Altai Mountains — a stunning natural landscape.",
      "Burabay — Kazakhstan’s ‘Little Switzerland’.",
      "Charyn Canyon — a natural and historical monument."
    ]
  };

  const slides = [
    { src: "https://avatars.mds.yandex.net/i?id=447256547577cf1aa1dbe8bfffeb4d43c784662f-4306866-images-thumbs&n=13", name: "Алтай таулары" },
    { src: "https://avatars.mds.yandex.net/i?id=88e86dbf49faac72671b826cd8e428f9b42c6c9a-16509561-images-thumbs&n=13", name: "Бурабай" },
    { src: "https://cdn.nur.kz/images/1200x675/fd1d9d9b2ac051cd.jpeg?version=1", name: "Шарын шатқалы" }
  ];

  const headings = {
    kk: { site: "Қазақстандағы Танымал Туристік Орындар", gallery: "Галерея", filter: "Фильтр", slider: "Танымал орындар", fav: "Сүйікті орындар", map: "Қазақстан картасы" },
    ru: { site: "Популярные туристические места Казахстана", gallery: "Галерея", filter: "Фильтр", slider: "Популярные места", fav: "Избранные места", map: "Карта Казахстана" },
    en: { site: "Popular Tourist Places in Kazakhstan", gallery: "Gallery", filter: "Filter", slider: "Popular Places", fav: "Favorite Places", map: "Map of Kazakhstan" }
  };

  const authorsText = {
    kk: { copyright: "© 2025 Барлық құқықтар қорғалған", authorsTitle: "Авторлар", authors: ["Таупық Нұрислам: репозиторий мен логика үшін жауапты", "Торыбай Нұрислам: дизайн және сайт"], button: "Авторлар" },
    ru: { copyright: "© 2025 Все права защищены", authorsTitle: "Авторы", authors: ["Таупык Нурислам: отвечал за репозиторий и логику", "Торыбай Нурислам: отвечал за дизайн и сайт"], button: "Авторы" },
    en: { copyright: "© 2025 All rights reserved", authorsTitle: "Authors", authors: ["Taupyk Nurislam: responsible for repository and logic", "Torybay Nurislam: responsible for design and site"], button: "Authors" }
  };

  // ==================== СЛАЙДЕР ====================
  const slideImage = document.getElementById('slide-image');
  const slideCaption = document.getElementById('slide-caption');
  const prevBtn = document.getElementById('prev');
  const nextBtn = document.getElementById('next');

  function showSlide(index) {
    slideIndex = ((index % slides.length) + slides.length) % slides.length;
    slideImage.src = slides[slideIndex].src;
    // если текстов меньше, подстрахуем
    const captionArray = sliderText[currentLang] || sliderText['kk'];
    slideCaption.textContent = captionArray[slideIndex] || slides[slideIndex].name || '';
  }

  prevBtn && prevBtn.addEventListener('click', () => showSlide(slideIndex - 1));
  nextBtn && nextBtn.addEventListener('click', () => showSlide(slideIndex + 1));
  showSlide(0);

  // автопрокрутка (необязательно) — оставил, но закомментирован
  // let slideTimer = setInterval(() => showSlide(slideIndex + 1), 6000);

  // ==================== ИЗБРАННОЕ ====================
  let favorites = JSON.parse(localStorage.getItem('favorites')) || [];

  function updateFavorites() {
    favList.innerHTML = '';
    favorites.forEach(name => {
      const li = document.createElement('li');
      li.textContent = name;
      li.dataset.name = name;

      const btn = document.createElement('button');
      btn.textContent = '×';
      btn.className = 'fav-remove';
      btn.setAttribute('aria-label', 'удалить из избранного');

      li.appendChild(btn);
      favList.appendChild(li);
    });
  }

  updateFavorites();

  places.forEach(place => {
    place.addEventListener('click', () => {
      const name = place.dataset.name;
      if (!favorites.includes(name)) {
        favorites.push(name);
        localStorage.setItem('favorites', JSON.stringify(favorites));
        updateFavorites();
      }
    });
  });

  favList.addEventListener('click', (e) => {
    if (e.target.classList.contains('fav-remove')) {
      const li = e.target.closest('li');
      const name = li.dataset.name;
      favorites = favorites.filter(n => n !== name);
      localStorage.setItem('favorites', JSON.stringify(favorites));
      updateFavorites();
    }
  });

  // ==================== ФИЛЬТР ====================
  const filterButtons = document.querySelectorAll('#filter button');
  filterButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const type = btn.dataset.type;
      places.forEach(place => {
        const placeType = place.dataset.type;
        place.style.display = (type === 'all' || type === placeType) ? 'block' : 'none';
      });
      // подсветка кнопки фильтра (необязательно)
      filterButtons.forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
    });
  });

  // ==================== КАРТА ====================
  // Проверяем, есть ли leaflet в странице
  if (window.L && document.getElementById('mapid')) {
    const map = L.map('mapid').setView([48.0, 66.9], 5);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

    const markers = [
      { coords: [49.5, 86.0], name: 'Алтай таулары' },
      { coords: [53.0, 71.5], name: 'Бурабай' },
      { coords: [43.2, 79.1], name: 'Шарын шатқалы' },
      { coords: [45.5, 79.2], name: 'Көлсай көлдері' },
      { coords: [43.2, 76.9], name: 'Алматы' },
      { coords: [43.0, 78.5], name: 'Көлтаз' },
      { coords: [51.2, 71.4], name: 'Астана' },
      { coords: [43.5, 69.8], name: 'Қаратау' },
      { coords: [45.6, 63.3], name: 'Байқоңыр' }
    ];

    markers.forEach(m => L.marker(m.coords).addTo(map).bindPopup(m.name));
  }

  // ==================== ЯЗЫК ====================
  function updateGallery(lang) {
    places.forEach(place => {
      const name = place.dataset.name;
      const p = place.querySelector('p');
      // если перевод есть — ставим, иначе оставляем существующий текст
      if (galleryText[lang] && galleryText[lang][name]) {
        p.textContent = galleryText[lang][name];
      }
    });
  }

  function updateAuthors(lang) {
    const t = authorsText[lang];
    if (!t) return;
    const copyright = document.getElementById('copyright-text');
    const authorsTitle = document.getElementById('authors-title');
    const authorsList = document.getElementById('authors-list');
    if (copyright) copyright.textContent = t.copyright;
    if (authorsTitle) authorsTitle.textContent = t.authorsTitle;
    if (authorsList) authorsList.innerHTML = t.authors.map(a => `<li>${a}</li>`).join('');
    if (authorsBtn) authorsBtn.textContent = t.button;
  }

  function setLanguage(lang) {
    currentLang = lang;
    const t = headings[lang] || headings['kk'];

    const siteTitle = document.getElementById('site-title');
    const galleryTitle = document.getElementById('gallery-title');
    const filterTitle = document.getElementById('filter-title');
    const sliderTitle = document.getElementById('slider-title');
    const favTitle = document.getElementById('fav-title');
    const mapTitle = document.getElementById('map-title');

    siteTitle && (siteTitle.textContent = t.site);
    galleryTitle && (galleryTitle.textContent = t.gallery);
    filterTitle && (filterTitle.textContent = t.filter);
    sliderTitle && (sliderTitle.textContent = t.slider);
    favTitle && (favTitle.textContent = t.fav);
    mapTitle && (mapTitle.textContent = t.map);

    updateGallery(lang);
    updateAuthors(lang);
    showSlide(slideIndex);

    langButtons.forEach(b => b.classList.toggle('active', b.dataset.lang === lang));
  }

  langButtons.forEach(btn => {
    btn.addEventListener('click', () => setLanguage(btn.dataset.lang));
  });

  // ==================== АВТОРЫ (МОДАЛ) ====================
  if (authorsBtn && authorsModal && closeBtn) {
    authorsBtn.addEventListener('click', () => authorsModal.style.display = 'block');
    closeBtn.addEventListener('click', () => authorsModal.style.display = 'none');
    window.addEventListener('click', e => { if (e.target === authorsModal) authorsModal.style.display = 'none'; });
  }

  // ==================== ИНФО О ГАЛЕРЕЕ ====================
  // создаём маленькое информационное окно под заголовком галереи при нажатии
  function createGalleryInfoBox() {
    // если уже создан — не дублируем
    if (document.getElementById('gallery-info-box')) return;

    const box = document.createElement('div');
    box.id = 'gallery-info-box';
    box.style.cssText = `
      max-width: 900px;
      margin: 12px auto 24px;
      padding: 14px 18px;
      border-radius: 12px;
      background: rgba(255,255,255,0.95);
      box-shadow: 0 6px 20px rgba(0,0,0,0.08);
      color: var(--text-color);
      border: 1px solid rgba(0,0,0,0.06);
      font-weight: 500;
      line-height: 1.4;
    `;

    // текст в зависимости от языка
    function getInfoText(lang) {
      if (lang === 'ru') {
        return 'Ниже — коллекция популярных туристических мест Казахстана. Нажмите на карточку, чтобы добавить место в «Избранное». Используйте фильтр для быстрого поиска по типу места.';
      } else if (lang === 'en') {
        return 'Below is a collection of popular tourist places in Kazakhstan. Click a card to add the place to "Favorites". Use the filter to quickly find places by type.';
      } else {
        return 'Төменде — Қазақстанның танымал туристік орындарының тізімі. Карточкаға басып, орынды «Сүйікті» тізімге қосыңыз. Түрі бойынша іздеу үшін фильтрді пайдаланыңыз.';
      }
    }

    box.innerHTML = `
      <div id="gallery-info-text">${getInfoText(currentLang)}</div>
      <div style="text-align:right;margin-top:8px;">
        <button id="gallery-info-close" style="padding:6px 12px;border-radius:8px;border:none;cursor:pointer;background:var(--btn-active);color:#fff;font-weight:600;">OK</button>
      </div>
    `;

    // вставляем после заголовка галереи
    if (galleryTitleEl && galleryTitleEl.parentNode) {
      galleryTitleEl.parentNode.insertBefore(box, galleryTitleEl.nextSibling);
      const close = document.getElementById('gallery-info-close');
      close && close.addEventListener('click', () => box.remove());
    }
  }

  // при клике на заголовок — показываем инфо (если захотят ещё раз — можно вызвать)
  galleryTitleEl && galleryTitleEl.addEventListener('click', createGalleryInfoBox);

  // также при смене языка — обновляем текст в инфо-боксе, если он виден
  function refreshGalleryInfoText() {
    const info = document.getElementById('gallery-info-box');
    if (info) {
      const txtEl = info.querySelector('#gallery-info-text');
      if (txtEl) {
        if (currentLang === 'ru') {
          txtEl.textContent = 'Ниже — коллекция популярных туристических мест Казахстана. Нажмите на карточку, чтобы добавить место в «Избранное». Используйте фильтр для быстрого поиска по типу места.';
        } else if (currentLang === 'en') {
          txtEl.textContent = 'Below is a collection of popular tourist places in Kazakhstan. Click a card to add the place to "Favorites". Use the filter to quickly find places by type.';
        } else {
          txtEl.textContent = 'Төменде — Қазақстанның танымал туристік орындарының тізімі. Карточкаға басып, орынды «Сүйікті» тізімге қосыңыз. Түрі бойынша іздеу үшін фильтрді пайдаланыңыз.';
        }
      }
    }
  }

  // переопределяем setLanguage, чтобы обновить инфо-текст
  const originalSetLanguage = setLanguage;
  setLanguage = function(lang) {
    originalSetLanguage(lang);
    refreshGalleryInfoText();
  };

  // ==================== ТЕМЫ ====================
  const themes = {
    dark: {
      '--bg-color': '#121212',
      '--text-color': '#eee',
      '--header-bg': '#1f1f1f',
      '--header-text': '#fff',
      '--btn-bg': '#333',
      '--btn-active': '#bb86fc',
      '--slider-bg': '#1e1e1e',
      '--slider-text': '#bb86fc',
      '--favorites-bg': '#1e1e1e',
      '--favorites-text': '#bb86fc'
    },
    light: {
      '--bg-color': '#e3f6ff',
      '--text-color': '#222',
      '--header-bg': 'linear-gradient(90deg, #009688, #26a69a)',
      '--header-text': '#fff',
      '--btn-bg': 'rgba(20,20,35,0.7)',
      '--btn-active': '#9d4cff',
      '--slider-bg': '#ffffff',
      '--slider-text': '#00695c',
      '--favorites-bg': '#ffffff',
      '--favorites-text': '#004d40'
    },
    ocean: {
      '--bg-color': '#d0f0fd',
      '--text-color': '#03396c',
      '--header-bg': '#0077b6',
      '--header-text': '#fff',
      '--btn-bg': '#00b4d8',
      '--btn-active': '#023e8a',
      '--slider-bg': '#caf0f8',
      '--slider-text': '#0077b6',
      '--favorites-bg': '#ade8f4',
      '--favorites-text': '#023e8a'
    },
    sunset: {
      '--bg-color': '#fff0e6',
      '--text-color': '#5c1a00',
      '--header-bg': '#ff7f50',
      '--header-text': '#fff',
      '--btn-bg': '#ffb347',
      '--btn-active': '#ff4500',
      '--slider-bg': '#ffe6d9',
      '--slider-text': '#b34700',
      '--favorites-bg': '#ffd9b3',
      '--favorites-text': '#b34700'
    }
  };

  function setTheme(themeName) {
    const theme = themes[themeName];
    if (!theme) return;
    Object.keys(theme).forEach(key => {
      document.documentElement.style.setProperty(key, theme[key]);
    });
    // подсветка активной кнопки
    themeButtons.forEach(b => b.classList.remove('active'));
    const btn = document.querySelector(`.theme-btn[data-theme="${themeName}"]`);
    if (btn) btn.classList.add('active');
  }

  themeButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      setTheme(btn.dataset.theme);
    });
  });

  // Тема по умолчанию
  if (document.querySelector('.theme-btn[data-theme="dark"]')) {
    setTheme('dark');
  } else {
    // если нет кнопки dark — поставить light
    setTheme('light');
  }

  // ==================== ДОПОЛНИТЕЛЬНЫЕ УДОБСТВА ====================
  // Обработчик клика по местам: при клике также показываем краткую карточку (всплывающую подсказку)
  places.forEach(place => {
    place.addEventListener('mouseenter', () => {
      // можно расширить — например, показывать подсказку
      place.style.filter = 'brightness(0.98)';
    });
    place.addEventListener('mouseleave', () => {
      place.style.filter = '';
    });
  });

  // Обработка клавиатуры для модальных окон (Esc закрывает)
  window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      if (authorsModal) authorsModal.style.display = 'none';
      const info = document.getElementById('gallery-info-box');
      if (info) info.remove();
    }
  });

  // ==================== ИНИЦИАЛИЗАЦИЯ ====================
  setLanguage(currentLang);
  updateGallery(currentLang);
  updateAuthors(currentLang);
  showSlide(0);
});

